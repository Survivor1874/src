    ...
    final Node<K, V>[] resize() {
        ...
        if (oldTab != null) {
            for (int j = 0; j < oldCap; ++j) {
                ...
                if ((currentNode = oldTab[j]) != null) {
                    oldTab[j] = null;
                    if (currentNode.next == null) {
                        newTab[currentNode.hash & (newCap - 1)] = currentNode;
                    } else if (currentNode instanceof TreeNode) {
                        ((TreeNode<K, V>) currentNode).split(this, newTab, j, oldCap);
                    } else { // preserve order
